#!/usr/bin/env bun
/**
 * generate — turn the Smithy JSON model in .generated-specs into the Intercom
 * Effect SDK.
 *
 * Input:  .generated-specs/intercom.json  (written by scripts/convert.ts)
 * Output: src/services/intercom.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Intercom's provider spec.
 * Intercom keeps the wire's snake_case member names on the TS surface, so no
...
/** Intercom's provider spec for the shared smithy→SDK compiler. */
const intercomSpec: SdkSpec = {
  ...
  // Sensitive strings (access tokens, secrets): the schema member carries
  ...
  // names for Intercom), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    contextType: "IntercomOpContext",
    commonErrorType: "IntercomOpError",
    commonErrorClasses: ["UnknownIntercomError"],
    protocol: "IntercomProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-intercom)",
...
runGeneratorCli({
  description: "Generate the Intercom Effect SDK from the Smithy model",
