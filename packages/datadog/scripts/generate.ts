 ...
 * Datadog Effect SDK.
 *
 * Input:  .generated-specs/{v1,v2}.json  (written by scripts/convert.ts)
 * Output: src/services/{v1,v2}.ts  +  src/services/index.ts
 *
 * The smithy→SDK compiler and CLI pipeline live in
 * `@distilled.cloud/core/codegen`; this script is Datadog's provider spec.
 * Datadog keeps the wire's snake_case member names on the TS surface, so no
...
const datadogSpec: SdkSpec = {
  ...
  // names for Datadog), so no runtime case discrimination is needed.
  ...
  operationDecl: {
    ...
    commonErrorClasses: ["UnknownDatadogError"],
    protocol: "DatadogProtocol",
    retry: "Retry.Retry",
  },

  sourceNote: ".generated-specs (specs/spec-mirror-datadog)",
